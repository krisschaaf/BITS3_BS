...
int main(int argc, char **argv)
{
    ...
    //Abhängig von der Anzahl der Parameter wird der Seitenersetzungsalgorithmus ausgewählt 
    ...
    while (1)
    {
        struct msg m = waitForMsg(); // This function blocks until a message from vmapp has arrived.
        switch (m.cmd)
        {
        case CMD_PAGEFAULT: //#define CMD_PAGEFAULT		1	// value gibt die einzulagernde Page mit
            allocate_page(m.value, m.g_count);
            break;
        case CMD_TIME_INTER_VAL: //#define CMD_TIME_INTER_VAL   	2	// Ein Time Interval ist abgelaufen
            if (pageRepAlgo == find_remove_aging)
            {
                update_age_reset_ref();
            }
            break;
        default: //#define CMD_ACK 		3	// value hat keine Bedeutung
            TEST_AND_EXIT(true, (stderr, "Unexpected command received from vmapp\n"));
        }
        sendAck(); // This function sends an ACK to vmapp
    }
    return 0;
}
...
void vmem_init(void)
{
    ...
    /*  
    Größe: VMEM_PHYSMEMSIZE
    Rahmengröße: VMEM_PAGESIZE
    Anzahl Seitenrahmen: VMEM_NFRAMES 
    */
    
    /*TODO Attach shared memory to vmem (virtual memory) */
...
{
    //TODO durch pageTable iterieren und die Seite mit der kleinsten Frame Nummer finden
    //  der R Bit dieser Seite darf nicht gesetzt sein also flags != 4 
    int smallestFrame = -1;

    for(int i = 0; i< VMEM_NPAGES; i++)    {
        if (vmem->pt[i].flags != 4)    //FIXME Zeiger richtig konfiguieren
        {
            if (vmem->pt[i].frame < smallestFrame || smallestFrame == -1)
            {
                smallestFrame = vmem->pt[i].frame;
            }            
        }        
    }
    if (smallestFrame == -1)    {
        // TODO wenn alle Frames benutzt werden gebe VOID_IDX zurück
        return VOID_IDX;
    } else {
...
{
    //TODO weist req_page dem nächsten freien Rahmen im mainMemory zu 
    int freeFrame = find_unused_frame();

    if(freeFrame == VOID_IDX)   {
        /* call correspoding page replacement algorithm */
    }

    //TODO update pageTable
    
    //TODO schreibe den PageFault in den logger 
    struct logevent le;
...
{
    // TODO lösche Seite von der Platte (pageFile) und schreibe sie in den mainMemory
    fetch_page_from_pagefile(page, frame);  //FIXME param frame sollte eigentlich ein Pointer sein 


    // TODO schreibe Seite in pageTable
    // TODO rufe logger() auf
}
...
{
    store_page_to_pagefile()
    // TODO lösche modifizierte Seite vom mainMemory und schreibe sie an richtige Stelle
    // auf Platte (pageFile)
    // TODO lösche Seite aus pageTable
    // TODO rufe logger() auf
}
